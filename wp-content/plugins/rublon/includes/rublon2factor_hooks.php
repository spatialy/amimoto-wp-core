...
function rublon2factor_authenticate($user, $username, $password) {
	...
	if (is_wp_error($user)) {	    
		...
		if (is_user_member_of_blog($user_id) && RublonHelper::isSiteRegistered() && !RublonHelper::isXMLRPCEnabled()) {
...
function rublon2factor_modify_login_form() {
        ...
        echo '<div style="display: block;" id="rublon-badge">
                <div class="rublon-badge-link">
                    <div id="RublonBadgeWidget" data-badge-widget-version="'.RublonHelper::BADGE_WIDGET_VERSION.'">                        
                        <div id="rublon-badge-img"></div>    
                    </div>
                </div>
              </div>';
